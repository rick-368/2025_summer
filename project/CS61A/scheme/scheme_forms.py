...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        signature_name = signature.first  
        formals = signature.rest
        body = expressions.rest
        lambda1 = do_lambda_form(Pair(formals, body), env)
        env.define(signature_name, lambda1)
...
def make_let_frame(bindings, env):
    ...
    if validate_formals(names):
        raise SchemeError('duplicate names in let bindings')
